...
class _CreateImagePageState extends State<CreateImagePage> {
  ...
  void _saveImageItem() async {
    ...
    if (provider.selectedImagePath != null &&
      ...
      _showOverlayMessage(context, '이미지, 이름, 음성 파일을 모두 입력해주세요.');
    }
  }
...
class _VoiceRecordWidgetState extends State<VoiceRecordWidget> {
  ...
  void stopRecording() async {
    ...
    widget.onAudioFilePathUpdated(_recordingFilePath);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('녹음 파일이 저장되었습니다.: $_recordingFilePath')),
    );
  }
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              IconButton(
                icon: Icon(Icons.send, color: isRecorded ? Color(0xFFEC295D) : Colors.grey),
                onPressed: isRecorded
                    ? () {
                  // 로그를 찍어서 확인합니다.
                  print('Send button pressed with file path: $_recordingFilePath');
                  widget.onAudioFilePathUpdated(_recordingFilePath);
                }
                    : null,
              ),
              // IconButton(
              //   icon: Icon(Icons.send, color: isRecorded ? Color(0xFFEC295D) : Colors.grey),
              //   onPressed: isRecorded ? () => widget.onAudioFilePathUpdated(_recordingFilePath) : null,
              // ),
            ],
          ),
          SizedBox(height: 20),
